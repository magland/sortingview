...
def create_position_pdf_plot(*, start_time_sec: np.float32, sampling_frequency: np.float32, pdf: np.ndarray, label: str):
    ...
    data = {"type": "PositionPdfPlot", "pdf": B, "samplingFrequency": sampling_frequency, "startTimeSec": start_time_sec}
    return Figure(data=data, label=label)


def create_live_position_pdf_plot(
    *,
    start_time_sec: np.float32,
    end_time_sec: np.float32,
    sampling_frequency: np.float32,
    num_positions: int,
    pdf_object: dict,
    segment_size: int,
