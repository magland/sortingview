...
def average_waveforms_view(self, *, unit_ids: List[int], label: Union[str, None] = None):
    ...
    for unit_id in unit_ids:
        ...
        channel_ids = self.get_unit_channel_neighborhood(unit_id=unit_id).tolist()
        plots.append(vv.AverageWaveformItem(unit_id=unit_id, channel_ids=channel_ids, waveform=waveform.T, waveform_std_dev=waveform_std_dev.T))
    ...
    return vv.AverageWaveforms(average_waveforms=plots, channel_locations=channel_locations)
