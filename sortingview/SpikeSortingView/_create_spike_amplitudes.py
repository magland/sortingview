...
def create_spike_amplitudes(self, *, unit_ids: List[int], label: Union[str, None] = None, _subsample_max_firing_rate: Union[float, None] = None):
    ...
    for unit_id in unit_ids:
        ...
        units.append({"unitId": unit_id, "spikeTimesSec": spike_times_sec.astype(np.float32), "spikeAmplitudes": spike_amplitudes.astype(np.float32)})

    # traces_sample = self.get_traces_sample(segment=0)

    data = {"type": "SpikeAmplitudes", "startTimeSec": 0, "endTimeSec": self.num_frames / self.sampling_frequency, "units": units}
