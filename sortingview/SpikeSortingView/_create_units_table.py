...
def create_units_table(self, *, unit_ids: List[int], unit_metrics: Union[List[dict], None] = None, label: Union[str, None] = None):
    ...
    columns = [
        {"key": "unitId", "label": "Unit", "dtype": "int"},
        {"key": "numEvents", "label": "Num. events", "dtype": "int"},
        {"key": "firingRateHz", "label": "Firing rate (Hz)", "dtype": "float"},
        {"key": "snr", "label": "SNR", "dtype": "float"},
    ]
    ...
    for unit_id in unit_ids:
        ...
        values = {"unitId": unit_id, "numEvents": len(spike_train), "firingRateHz": len(spike_train) / (self.num_frames / self.sampling_frequency), "snr": snr}
        if unit_metrics is not None:
            for u in unit_metrics:
                d = u["data"].get(str(unit_id), None)
                if d is not None:
                    values[f'metric:{u["name"]}'] = d
        rows.append({"unitId": unit_id, "values": values})
    data = {"type": "UnitsTable", "columns": columns, "rows": rows}
