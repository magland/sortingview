...
class UnitMetricsGraph(View):
    ...
    def __init__(self, *, metrics: List[UnitMetricsGraphMetric], units: List[UnitMetricsGraphUnit], **kwargs) -> None:
        super().__init__("UnitMetricsGraph", **kwargs)
        self._metrics = metrics
        self._units = units

    def to_dict(self) -> dict:
        ret = {"type": self.type, "units": [a.to_dict() for a in self._units], "metrics": [a.to_dict() for a in self._metrics]}
        return ret
