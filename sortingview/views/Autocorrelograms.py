...
class AutocorrelogramItem:
    ...
    def to_dict(self):
        return {"unitId": self.unit_id, "binEdgesSec": self.bin_edges_sec, "binCounts": self.bin_counts}
...
class Autocorrelograms(View):
    ...
    def __init__(self, autocorrelograms: List[AutocorrelogramItem], **kwargs) -> None:
        super().__init__("Autocorrelograms", **kwargs)
        self._autocorrelograms = autocorrelograms

    def to_dict(self) -> dict:
        ret = {"type": self.type, "autocorrelograms": [a.to_dict() for a in self._autocorrelograms]}
        return ret
