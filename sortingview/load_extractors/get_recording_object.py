...
def get_recording_object(recording: si.BaseRecording):
    if hasattr(recording, "sortingview_object"):
        ...
        recording_object = {"recording_format": "nwb2", "data": {"nwb_file_uri": nwb_file_uri, "electrical_series_name": electrical_series_name}}
        ...
        data["file_paths"] = [kcl.store_file_local(str(folder_path / file_path), label=os.path.basename(file_path), reference=True) for file_path in file_paths]  # important
        data["channel_locations"] = recording.get_channel_locations()
        recording_object = {"recording_format": "BinaryRecordingExtractor", "data": data}
    elif isinstance(recording, si.ConcatenateSegmentRecording):
        data = {"recording_list": [get_recording_object(r) for r in recording.recording_list]}
        recording_object = {"recording_format": "ConcatenateSegmentRecording", "data": data}
        ...
        recording_object = {"recording_format": "BandpassFilterRecording", "data": data}
    else:
        print(recording)
