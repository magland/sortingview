...
export const useTimeseriesSelectionInitialization = (start: number, end: number, timeOffset=0) => {
    ...
    }, [timeseriesSelection.timeseriesStartTimeSec, timeseriesSelection.timeseriesEndTimeSec, timeseriesSelectionDispatch, start, end, timeOffset])
}
...
export const useTimeRange = (timestampOffset=0) => {
    ...
    return {
        visibleStartTimeSec: timeseriesSelection.visibleStartTimeSec !== undefined ? timeseriesSelection.visibleStartTimeSec - timestampOffset : undefined,
        visibleEndTimeSec: timeseriesSelection.visibleEndTimeSec !== undefined ? timeseriesSelection.visibleEndTimeSec - timestampOffset : undefined,
...
export const useTimeseriesSelection = () => {
    ...
    const timeForFraction = useCallback((fraction: number) => {
        const window = (timeseriesSelection.visibleEndTimeSec || 0) - (timeseriesSelection.visibleStartTimeSec || 0)
...
export type TimeseriesSelectionAction = InitializeTimeseriesSelectionTimesAction  | PanTimeseriesSelectionAction
    | PanTimeseriesSelectionDeltaTAction | ZoomTimeseriesSelectionAction | SetVisibleTimeRangeAction | SetFocusTimeTimeseriesSelectionAction | SetFocusTimeIntervalTimeseriesSelectionAction | SetSelectedElectrodeIdsTimeseriesSelectionAction
...
const initializeTimeseriesSelectionTimes = (state: TimeseriesSelection, action: InitializeTimeseriesSelectionTimesAction): TimeseriesSelection => {
    ...
    const newState: TimeseriesSelection = {
        ...
        visibleEndTimeSec: state.visibleEndTimeSec === undefined ? newEnd : state.visibleEndTimeSec,
...
const panTimeHelper = (state: TimeseriesSelection, panDisplacementSeconds: number) => {
    if (state.visibleStartTimeSec === undefined || state.visibleEndTimeSec === undefined || state.timeseriesStartTimeSec === undefined || state.timeseriesEndTimeSec === undefined) {
    ...
    if (panDisplacementSeconds > 0) {
        ...
        newEnd = Math.min(state.visibleEndTimeSec + panDisplacementSeconds, state.timeseriesEndTimeSec)
        ...
        newStart = Math.max(state.visibleStartTimeSec + panDisplacementSeconds, state.timeseriesStartTimeSec)
...
const zoomTime = (state: TimeseriesSelection, action: ZoomTimeseriesSelectionAction): TimeseriesSelection => {
    ...
    const totalTimeseriesLength = state.timeseriesEndTimeSec - state.timeseriesStartTimeSec
    ...
    if (autoScrollVisibleTimeRange) {
        if ((state.visibleStartTimeSec !== undefined) && (state.visibleEndTimeSec !== undefined)) {
            if ((currentTimeSec < state.visibleStartTimeSec) || (currentTimeSec > state.visibleEndTimeSec)) {
                ...
                if (newState.visibleStartTimeSec < (state.timeseriesStartTimeSec || 0)) {
                    const delta = (state.timeseriesStartTimeSec || 0) - newState.visibleStartTimeSec
                    newState.visibleStartTimeSec += delta
                    newState.visibleEndTimeSec += delta
                }
            }
...
const setSelectedElectrodeIds = (state: TimeseriesSelection, action: SetSelectedElectrodeIdsTimeseriesSelectionAction): TimeseriesSelection => {
