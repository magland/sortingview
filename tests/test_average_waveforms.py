...
def main():
    ...
    view = test_average_waveforms(recording=recording, sorting=sorting)
...
def test_average_waveforms(*, recording: si.BaseRecording, sorting: si.BaseSorting):
    ...
    for unit_id in sorting.get_unit_ids():
        ...
        waveform = a["waveform"]
        average_waveform_items.append(vv.AverageWaveformItem(unit_id=unit_id, waveform=waveform.T, channel_ids=channel_ids))
    channel_locations = {}
    ...
    view = vv.AverageWaveforms(average_waveforms=average_waveform_items, channel_locations=channel_locations, show_reference_probe=True)
    return view
...
def compute_average_waveform(*, recording: si.BaseRecording, sorting: si.BaseSorting, unit_id: int):
    ...
    return {"channel_ids": recording.get_channel_ids().astype(np.int32), "waveform": waveform.astype(np.float32)}
