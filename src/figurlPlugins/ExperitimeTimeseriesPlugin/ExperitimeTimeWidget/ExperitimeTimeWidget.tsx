...
const ExperitimeTimeWidget = (props: Props) => {
    ...
    // schedule repaint when panels or time range changes
    useEffect(() => {
        if (mainLayer) {
            mainLayer.scheduleRepaint()
        }
    }, [panels, mainLayer, selection.timeRange])
    ...
    }, [selection.currentTimepoint, selection.timeRange, setSpanWidgetInfo, timeseriesStartTime, timeseriesEndTime])
